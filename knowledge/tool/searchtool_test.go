...
func TestAgenticFilterSearchTool(t *testing.T) {
	...
	t.Run("verify declaration metadata", func(t *testing.T) {
		...
		require.NotNil(t, decl.InputSchema)
		require.NotNil(t, decl.OutputSchema)
		require.Contains(t, decl.InputSchema.Properties, "filter")
		filterSchema := decl.InputSchema.Properties["filter"]
		require.Equal(t, "object", filterSchema.Type)
		require.NotContains(t, filterSchema.Required, "operator")
		require.Contains(t, filterSchema.Properties, "operator")
		require.Contains(t, filterSchema.Properties, "value")
		require.Contains(t, filterSchema.Properties["operator"].Enum, searchfilter.OperatorEqual)
		require.Contains(t, filterSchema.Properties["operator"].Enum, searchfilter.OperatorAnd)
		require.Contains(t, filterSchema.Properties["value"].Description, "array of nested filter conditions")
	})
	...
	t.Run("verify options", func(t *testing.T) {
		...
		decl = searchTool.Declaration()
		require.Contains(t, decl.Description, customDesc)
		require.Contains(t, decl.Description, "== FILTER GUIDANCE ==")
		require.Contains(t, decl.Description, "Available filters:")
		require.NotContains(t, decl.Description, "tool description:")
		require.NotContains(t, decl.Description, "filter info:")
...
func TestKnowledgeSearchRequestWithFilter_AutoGeneratedSchema(t *testing.T) {
	schema := internaltool.GenerateJSONSchema(reflect.TypeOf(KnowledgeSearchRequestWithFilter{}))
	require.NotNil(t, schema)

	schemaBytes, err := json.MarshalIndent(schema, "", "  ")
	require.NoError(t, err)
	t.Logf("auto-generated schema:\n%s", string(schemaBytes))
	...
	require.Equal(t, "string", schema.Properties["query"].Type)
	require.Equal(t, "object", schema.Properties["filter"].Type)
	require.Empty(t, schema.Defs)

	filterSchema := schema.Properties["filter"]
	require.NotNil(t, filterSchema)
	require.Equal(t, "object", filterSchema.Type)
	require.Contains(t, filterSchema.Description, "Filter conditions to apply to the search query")
	require.NotContains(t, filterSchema.Required, "operator")
	require.Contains(t, filterSchema.Properties, "field")
	require.Contains(t, filterSchema.Properties, "operator")
	require.Contains(t, filterSchema.Properties, "value")
	require.Equal(t, "string", filterSchema.Properties["field"].Type)
	require.Equal(t, "string", filterSchema.Properties["operator"].Type)
	require.Equal(t, "object", filterSchema.Properties["value"].Type)
	require.Contains(t, filterSchema.Description, "Use lowercase operators eq ne gt gte lt lte")
	require.Contains(t, filterSchema.Properties["operator"].Description, "Valid values are eq ne gt gte lt lte")
	require.Contains(t, filterSchema.Properties["value"].Description, "array of nested filter conditions")
}
