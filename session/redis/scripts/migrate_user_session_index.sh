...
for meta_pattern in "${meta_patterns[@]}"; do
  while IFS= read -r meta_key; do
    ...
    if [[ "${apply}" -eq 1 ]]; then
      result="$("${redis_cli[@]}" --eval "${lua_script}" "${meta_key}" , apply 2>/dev/null || true)"
    else
      result="$("${redis_cli[@]}" --eval "${lua_script}" "${meta_key}" , dry-run 2>/dev/null || true)"
    fi
  ...
  done < <("${redis_cli[@]}" --scan --pattern "${meta_pattern}" -i 0.01 2>/dev/null)
done

if [[ "${check_zset}" -eq 1 ]]; then
  for zset_pattern in "${zset_patterns[@]}"; do
    while IFS= read -r zset_key; do
      [[ -z "${zset_key}" ]] && continue
      zset_keys=$((zset_keys + 1))
      count="$("${redis_cli[@]}" HLEN "${zset_key}" 2>/dev/null || echo 0)"
      zset_sessions=$((zset_sessions + count))
      if [[ "${zset_keys}" -le 20 ]]; then
        echo "legacy zset key: ${zset_key}, sessions=${count}"
      fi
    done < <("${redis_cli[@]}" --scan --pattern "${zset_pattern}" -i 0.01 2>/dev/null)
  done
fi
